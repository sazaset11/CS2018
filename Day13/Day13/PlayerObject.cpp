...
namespace playerobject {
	...
	void move(S_PlayerObject *pObj, CHAR_INFO *pMapBuf, int _dx, int _dy) {
		...
		if (pMapBuf[_checky*80 + _checkx].Attributes == 0x0007) {
			pObj->m_Position.X = _checkx;
			pObj->m_Position.Y = _checky;
		}
		else if (pMapBuf[_checky*80 + _checkx].Attributes == 0x00c0) {
			printf("문이 잠겨있습니다.			");
		}
		else if (pMapBuf[_checky*80 + _checkx].Attributes == 0x00b0) {
			printf("열쇠가 떨어져있습니다.			");
		}
	}
	...
	int use(S_PlayerObject *pObj, CHAR_INFO *pMapBuf, int _dx, int _dy) {
		...
		if (pMapBuf[_checky*80 + _checkx].Attributes == 0x00c0) {
			if (!pObj->m_nKeyCount) {
				printf("열쇠가 필요합니다.");
			}
			
			pObj->m_nKeyCount--;
			pMapBuf[_checky*80 + _checkx].Attributes = 0x0007;

			printf("문이 열렸습니다..		");

			return 1;
		}
		else if (pMapBuf[_checky*80 + _checkx].Attributes == 0x00a0) {
			pObj->m_nKeyCount--;
			pMapBuf[_checky*80 + _checkx].Attributes = 0x0007;
	...
	void get(S_PlayerObject *pObj, CHAR_INFO *pMapBuf, int _dx, int _dy){
		...
		if (pMapBuf[_checky*80 + _checkx].Attributes == 0x00b0) {
			pObj->m_nKeyCount++;
			pMapBuf[_checky*80 + _checkx].Attributes = 0x0007;
